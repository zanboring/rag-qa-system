...
def _confidence_from_scores(scores: list[float]) -> list[float]:
    ...
    **已知局限（重要：不要把它当作「答案可信度」使用）**

    这个量的真实含义是「该片段在本次 Top-K 中的相对占比」，而不是「这个答案有多可信」。
    它有两个反直觉的性质：

    1. **单片段场景恒为 1.0**。当只召回 1 条片段时，无论其原始相似度是 0.9 还是 0.05，
       归一化后都得到 1.0——系统会对唯一一条、可能毫不相关的片段给出满分置信度。
    2. **分布均匀时偏低**。多个片段分数接近时每项接近 1/n，看起来"低置信"，
       但这可能恰恰是"多条片段都同样相关"的情况，并不代表不确定。

    因此它只能作为相对集中度的参考。如果要用作拒答判据，应当组合使用：
    原始绝对分数的阈值（需针对所选 embedding 模型标定）、Top-1 与 Top-2 的分差、
    以及多个 Top 片段结论的一致性。
    """
    total = sum(scores)
    if total <= 0:
